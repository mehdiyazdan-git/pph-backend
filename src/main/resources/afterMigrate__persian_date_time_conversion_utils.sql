...
create
    ...
    returns TABLE
            (
                cumulative_contract_amount                  bigint,
                cumulative_contract_quantity                bigint,
                contract_advanced_payment                   bigint,
                contract_performance_bond                   bigint,
                contract_insurance_deposit                  bigint,
                total_committed_contract_amount             bigint,
                total_remaining_contract_amount             bigint,
                total_committed_contract_count              bigint,
                total_invoice_count                         bigint,
                total_consumed_contract_advanced_payment    bigint,
                total_outstanding_contract_advanced_payment bigint,
                total_committed_performance_bond            bigint,
                total_remaining_performance_bond            bigint,
                total_committed_insurance_deposit           bigint,
                total_remaining_insurance_deposit           bigint
            )
    language plpgsql
as
$$
...
BEGIN
    RETURN QUERY WITH ContractTotals AS (SELECT SUM(ci.unit_price * ci.quantity)                       AS total_contract_amount,
                                         ...
                                         WHERE ci.contract_id = given_contract_id
                                         GROUP BY c.advance_payment, c.performance_bond, c.insurance_deposit),
                      InvoiceTotals AS (SELECT SUM(ii.unit_price * ii.quantity)                       AS total_invoice_amount,
...
create
    ...
    returns TABLE
            (
                report_date_persian character varying,
...
create
    ...
    returns TABLE
            (
                payment_date_persian character varying,
...
--18
create
    or replace function invoiced_not_invoiced_by_customer_code_and_year_name(p_customer_code character varying,
